...
bool ofApp::BFS()
{
    ...
    MazeQueue.push(curidx);
    curidx->visit = true;
    curidx->revisit = true;
    
    while (!path.empty()) {
        curidx = MazeQueue.front();
        MazeQueue.pop();

        vector<maze*> directions = { curidx->right, curidx->down, curidx->left, curidx->up };
        for (auto dir : directions) {
            if (dir != NULL && !dir->visit) {
                MazeQueue.push(dir);
                dir->parent = curidx;
                dir->visit = true;
